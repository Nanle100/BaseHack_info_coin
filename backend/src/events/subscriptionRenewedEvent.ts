...
export const listenToSubscriptionRenewedEvent = async () => {
  try {
    ...
    contract.on("SubscriptionRenewed", async (...args) => {
      try {
        ...
        console.error("Error in SubscriptionRenewed handler: ", error);
      }
    });

    console.log("Listening for SubscriptionRenewed events...");
  } catch (error: any) {
