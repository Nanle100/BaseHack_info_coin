...
contract DataPackageManager is ReentrancyGuard {
    ...
    {
        DataPackage storage dataPackage = dataPackages[_packageId];
        require(dataPackage.id != 0, "Data package does not exist");

        return (
            dataPackage.id,
            dataPackage.provider,
            dataPackage.price,
            dataPackage.dataAmount,
            dataPackage.isActive
        );
    }
